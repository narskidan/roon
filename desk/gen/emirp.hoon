|=  n=@ud
:: Library I cobbled together for efficiently 
:: working with primes. Including it here because 
:: the skeleton for this challenge doesn't include 
:: a /lib dir
=/  emirp-lib  
|%
  ++  is-dvzb
  ...
  ++  fix-dots
    ...
    `@ud`(scan n dem)
  --
:: Begin at the first emirp
=/  j  13
|-
:: Base case: we've found n emirps
?:  =(0 n)  0
=/  is-emirp
  :: Only prime numbers can be emirp
  ?.  (is-prime:emirp-lib j)  %.n
  =/  j-flud  (flud:emirp-lib j)
  :: Disqualify palindromes
  ?.  !=(j j-flud)  %.n
  (is-prime:emirp-lib j-flud)
:: Keep running sum of emirps
%+  add  ?:(is-emirp j 0)
%=  $
  n  ?:(is-emirp (sub n 1) n)
  j  (add 2 j)
==